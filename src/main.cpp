// ****************************************************************** 
// My name is inderjeet singh , 000745250 , I certify that this is my work and has not been shared to any class fellow in any way.
// COMP-10184 
// Mohawk College 
 
// library for Arduino framework  
#include <Arduino.h> 
// 1-Wire sensor communication libary 
#include <OneWire.h> 
// DS18B20 sensor library 
#include <DallasTemperature.h> 
 
// Pin that the  DS18B20 is connected to 
const int oneWireBus = D3;      
 
// Setup a oneWire instance to communicate with any OneWire devices 
OneWire oneWire(oneWireBus); 
 
// Pass our oneWire reference to Dallas Temperature sensor  
DallasTemperature DS18B20(&oneWire); 
DeviceAddress thermometer;
void convertAddress(DeviceAddress DeviceAddress){
  char addressString[17];
  for(uint8_t i = 0; i < 8 ; i++){
    if(thermometer[i] < 16) sprintf(addressString + (i * 2 ), "0");
    sprintf(addressString  + (i * 2), "%X", thermometer[i]);
  }
  Serial.println(addressString);
}
 
void setup() { 
  // configure the USB serial monitor 
  Serial.begin(115200); 
 
  // Start the DS18B20 sensor 
  DS18B20.begin(); 


Serial.println("/n");
Serial.println("/n");
Serial.println("/n");
Serial.println("/n");

Serial.println("Temperature Application");


 if(!DS18B20.getAddress(thermometer, 0)){
    Serial.println("NO DS18B20 TEM are installed"); 
    while(1);
  }
  
  else{
    Serial.println("DS18B20 TEM are installed"); 
    DS18B20.getAddress(thermometer, 0);
    convertAddress(thermometer);
  }
 
} 
 
void loop() { 
  float fTemp; 
 
  // ask DS18B20 for the current temperature 
  DS18B20.requestTemperatures(); 
 
  // fetch the temperature.  We only have 1 sensor, so the index is 0. 
  fTemp = DS18B20.getTempCByIndex(0); 
 

  if(fTemp  < 10 ) 
  Serial.println("Current temperature is: " + String(fTemp) + "° C or Cold!"); 
  else if(fTemp  > 10 && fTemp  < 15)
    Serial.println("Current temperature is: " + String(fTemp) + "° C or Cool"); 
   else if(fTemp  > 15 && fTemp  < 25 )
    Serial.println("Current temperature is: " + String(fTemp) + "° C or Perfect"); 
 else if(fTemp  > 25 && fTemp  < 30)
    Serial.println("Current temperature is: " + String(fTemp) + "° C or Warm"); 
 else if(fTemp  > 30 && fTemp  < 35)
    Serial.println("Current temperature is: " + String(fTemp) + "° C or Hot"); 
 else if(fTemp  > 35 ) 
  Serial.println("Current temperature is: " + String(fTemp) + "° C or Too Hot!");
 


  /// delay fo 5 Sec
  delay(5000); 
} 